...
def pre_process_images(raw_images_path):
    ...
    aligned_images = []
    for image_name in tqdm(sorted_images_names):
        try:
    ...
    for image, name in zip(aligned_images, sorted_images_names):
        print(image)
        real_name = name.split('.')[0]
        image.save(f'{paths_config.input_data_path}/processed/{real_name}.jpeg')
